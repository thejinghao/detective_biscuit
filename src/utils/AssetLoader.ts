import Phaser from 'phaser'
import { ASSET_KEYS } from '../constants/AssetKeys'

export class AssetLoader {
  static preloadAllAssets(scene: Phaser.Scene): void {
    // Biscuit sprites
    scene.load.image(ASSET_KEYS.BISCUIT.WALK1, '/assets/sprites/biscuit/walk/walk1.png')
    scene.load.image(ASSET_KEYS.BISCUIT.WALK2, '/assets/sprites/biscuit/walk/walk2.png')
    scene.load.image(ASSET_KEYS.BISCUIT.WALK3, '/assets/sprites/biscuit/walk/walk3.png')
    scene.load.image(ASSET_KEYS.BISCUIT.WALK4, '/assets/sprites/biscuit/walk/walk4.png')
    
    scene.load.image(ASSET_KEYS.BISCUIT.CONVO1, '/assets/sprites/biscuit/convo/convo1.png')
    scene.load.image(ASSET_KEYS.BISCUIT.CONVO2, '/assets/sprites/biscuit/convo/convo2.png')
    scene.load.image(ASSET_KEYS.BISCUIT.CONVO3, '/assets/sprites/biscuit/convo/convo3.png')
    scene.load.image(ASSET_KEYS.BISCUIT.CONVO4, '/assets/sprites/biscuit/convo/convo4.png')
    scene.load.image(ASSET_KEYS.BISCUIT.CONVO5, '/assets/sprites/biscuit/convo/convo5.png')
    scene.load.image(ASSET_KEYS.BISCUIT.CONVO6, '/assets/sprites/biscuit/convo/convo6.png')
    
    // UI assets
    scene.load.image(ASSET_KEYS.UI.START, '/assets/sprites/ui/start.png')
    scene.load.image(ASSET_KEYS.UI.NEXT, '/assets/sprites/ui/next.png')
    scene.load.image(ASSET_KEYS.UI.DIALOGUE_BOX, '/assets/sprites/ui/dialoguebox.png')
    scene.load.image(ASSET_KEYS.UI.TOILET_PAPER, '/assets/sprites/ui/toiletpaper.png')
    scene.load.image(ASSET_KEYS.UI.TOILET_PAPER_CHEWED, '/assets/sprites/ui/toiletpaper-chewed.png')
    
    // Tiles
    scene.load.image(ASSET_KEYS.TILES.PAWGRASS, '/assets/sprites/mesh/pawgrass.png')
    scene.load.image(ASSET_KEYS.TILES.GRASS, 'data:image/svg+xml;base64,')
    scene.load.image(ASSET_KEYS.TILES.STONE, 'data:image/svg+xml;base64,')
    
    // Projectiles
    scene.load.image(ASSET_KEYS.PROJECTILES.OLIVE, '/assets/sprites/projectile/olive.png')
    scene.load.image(ASSET_KEYS.PROJECTILES.POOP, '/assets/sprites/projectile/poop.png')
    
    // Backgrounds
    scene.load.image(ASSET_KEYS.BACKGROUNDS.FLOOR, '/assets/background/floor.png')
    
    // FPS Game assets
    scene.load.image(ASSET_KEYS.FPS.BACKGROUND, 'data:image/svg+xml;base64,')
    scene.load.image(ASSET_KEYS.FPS.CROSSHAIR, 'data:image/svg+xml;base64,')
    scene.load.image(ASSET_KEYS.FPS.BULLET, 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iOCIgaGVpZ2h0PSI4IiB2aWV3Qm94PSIwIDAgOCA4IiBmaWxsPSJub25lIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgo8Y2lyY2xlIGN4PSI0IiBjeT0iNCIgcj0iNCIgZmlsbD0iI0ZGRkYwMCIvPgo8L3N2Zz4K')
    scene.load.image(ASSET_KEYS.FPS.TARGET, 'data:image/svg+xml;base64,')
  }
}